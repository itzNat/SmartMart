...
class App {
  ...
  setupEventListeners() {
    ...
    document.getElementById('mobile-search-input').addEventListener('input', this.handleSearch.bind(this));
    
    // Filter and sort
    document.getElementById('tag-filter').addEventListener('change', this.handleTagFilter.bind(this));
    ...
    this.wishlistManager.setupWishlistEventListeners(this.cartManager, this.productsManager.productsData);
    ...
    document.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.wishlist-btn')) {
        ...
        this.wishlistManager.toggleWishlist(Number(id), this.productsManager.productsData);
      }
    });
  ...
  optionsContainer.addEventListener('click', function(e) {
    ...
    document.querySelectorAll('.custom-tag-select .option').forEach(opt => opt.classList.remove('active'));
  ...
  document.addEventListener('click', function(e) {
    ...
    document.querySelector(`.custom-tag-select .option[data-value="${initialValue}"]`).classList.add('active');
  }
  ...
  toggleView(view) {
    ...
    if (view === 'grid') {
      ...
      document.getElementById('products-container').classList.remove('products-list-view');
      document.getElementById('products-container').classList.add('grid', 'grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3', 'xl:grid-cols-4', 'gap-6');
    } else {
      ...
      document.getElementById('products-container').classList.remove('grid', 'grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3', 'xl:grid-cols-4', 'gap-6');
      document.getElementById('products-container').classList.add('products-list-view');
    }
  ...
  renderProducts() {
    ...
    const totalPages = Math.ceil(filteredProducts.length / this.productsManager.productsPerPage);
    
    // Calculate products for current page
    const startIndex = (this.productsManager.currentPage - 1) * this.productsManager.productsPerPage;
    ...
    if (productsToShow.length === 0) {
      ...
      productsToShow.forEach(product => {
        const isInWishlist = this.wishlistManager.wishlist.some(item => item.id === product.id);
        const productElement = document.createElement('div');
        productElement.innerHTML = this.productsManager.createProductElement(product, isInWishlist);
  ...
  renderPagination(totalProducts, totalPages) {
    ...
    // Previous button
    paginationHTML += `
      <button class="pagination-prev w-8 h-8 rounded-full glass-dark flex items-center justify-center ${this.productsManager.currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}" ${this.productsManager.currentPage === 1 ? 'disabled' : ''}>
    ...
    let startPage = Math.max(1, this.productsManager.currentPage - Math.floor(maxVisiblePages / 2));
    ...
    for (let i = startPage; i <= endPage; i++) {
      paginationHTML += `
        <button class="pagination-page w-8 h-8 rounded-full ${this.productsManager.currentPage === i ? 'bg-primary text-white' : 'glass-dark hover:bg-primary hover:text-white'} transition flex items-center justify-center" data-page="${i}">
          ${i}
        </button>
      `;
    }
    ...
    // Next button
    paginationHTML += `
      <button class="pagination-next w-8 h-8 rounded-full glass-dark flex items-center justify-center ${this.productsManager.currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''}" ${this.productsManager.currentPage === totalPages ? 'disabled' : ''}>
    ...
    document.querySelectorAll('.pagination-page').forEach(button => {
      button.addEventListener('click', (e) => {
        this.productsManager.currentPage = parseInt(e.target.getAttribute('data-page'));
        this.renderProducts();
    ...
    document.querySelector('.pagination-next').addEventListener('click', () => {
      const totalPages = Math.ceil(this.productsManager.getFilteredProducts().length / this.productsManager.productsPerPage);
